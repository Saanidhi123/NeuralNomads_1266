...
async def narrative(req: ReportRequest):
    defects = req.defects
    if not defects:
        return {"narrative": "No defects detected. Structure appears stable."}
    
    max_sss = max((d.get("sss", 1) for d in defects), default=1)
    summary = "\\n".join([
        f"- {d['class_name']} | SSS: {d['sss']} | Depth: {d['depth_max']:.2f} | Action: {d['action']}"
        for d in defects
    ])
...
Defects found:
{summary}
Maximum severity: {max_sss}/5

Write a 3-sentence plain-English assessment.
Mention defect types, severity, and recommend action.
